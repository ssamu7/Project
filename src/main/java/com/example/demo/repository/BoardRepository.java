...
// DB처리에 대한 어노테이션(Component)
@Repository
// BoardDataAccessObject(BoardA0)
public class BoardRepository {
    static final Logger log = LoggerFactory.getLogger(BoardRepository.class);


    //JdbcTemplate은 DB Query를 생성하는데 활용
    @Autowired
    private JdbcTemplate jdbcTemplate;

    public void create(Board board) throws Exception {
        //insert는 데이터를 입력함
        //board는 creat table로 만든 내용
        // board에 있는 title, content, writer에
        //특정값 3개를 삽입하기 위해 ?,?,?를 셋팅한 상태!
    ...
    public List<Board> list() throws Exception {
        ...
        //board, title... 등은 board에 있는 정보들
        //조건을 줄 때 where를 사용
        //여기서 조건은 board_no가 0보다 크다는 조건을 가지고 있음
        //order by는 정렬 조건에 해당
        //board_no와 reg_date에 대해 정렬 조건을 줬음
    ...
    public Board read(Integer boardNo) throws Exception {
        ...
        //처리한 결과가 아무것도 없다면, null을 리턴하고, 결과가 나왔다면 처리한 결과의 첫번째 행을 리턴한다
        //0은 첫번째 행을 의미함
        return results.isEmpty() ? null :  results.get(0);
    }

    public void remove(Integer boardNo) throws Exception {
        // DB테이블 안에 있는 내용을 지울때는 delete를 사용한다.
