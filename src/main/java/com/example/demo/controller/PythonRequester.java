...
public class PythonRequester {
    ...
    // RequestMapping value는 URL주소에 해당한다.
    // 그러므로 localhost:8080/doRequestPythonRest 가 URL 요청으로 둘어가면 아래 함수가 동작한다.
    @RequestMapping(
    ...
    public ModelAndView doRequestPythonRest(Model model) {
        ...
        // 처리 결과는 pyResult.html에서 확인하라는 의미
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.setViewName("pyResult");

        // RestFUL 서버에 요청을 보내 받아온 정보가 result인데 이 정보를 msg라는 속성값에 할당했다.
        // 그러므로 사용하고자 한다며 html코드에서 msg를 활용해야한다.
